...
Texture *gf3d_texture_from_surface(SDL_Surface *surface)
{
    ...
    if(!tex) 
    {
        slog("could not create a texture from surface");
        return NULL;
    }
    ...
    if(vkCreateImage(gf3d_texture.device, &imageInfo, 0, &tex->textureImage) != VK_SUCCESS)
    {
        slog("could not create VkImage");
        gf3d_texture_delete(tex);
        return NULL;
    }
    ...
    slog("created texture from surface");
