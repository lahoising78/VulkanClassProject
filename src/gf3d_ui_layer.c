...
#define _GF3D_UI_LAYER_MAX_COUNT_     32
...
void gf3d_ui_manager_close()
{
    ...
    if(ui_manager.layer_list)
    {
        for(i = 0; i < ui_manager.count; i++)
        {
            ...
            if(layer->renderer) SDL_DestroyRenderer(layer->renderer);
            layer->renderer = NULL;
            if(layer->surface) SDL_FreeSurface(layer->surface);
            layer->surface = NULL;

            if(layer->uiList)
            {
                gf3d_ui_free_list(layer);
                free(layer->uiList);
                layer->uiList = NULL;
            }
...
void gf3d_ui_manager_init_layers()
{
    ...
    for(i = 0; i < ui_manager.count; i++)
    {
        ...
        layer->count = _GF3D_UI_LAYER_MAX_COUNT_;
        layer->uiList = (UI*)gfc_allocate_array(sizeof(UI), layer->count);
        if(layer->uiList) memset(layer->uiList, 0, sizeof(UI) * layer->count);
    }
}
